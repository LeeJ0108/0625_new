...
def main():
    ...
    measurement_variance = 0.00095  # 측정 노이즈 공분산
    ...
    while not rospy.is_shutdown():  
        ...
        # error = 0.1
        # if ENC_vel >= last_ENC_VEL+round(p.IMU_vel_x*3.6,1)  - error and ENC_vel <= last_ENC_VEL+round(p.IMU_vel_x*3.6,1)  + error:
        #     ENC_vel = ENC_vel
        #     print("-----------",p.IMU_vel_x)

        # else:
        #     ENC_vel = last_ENC_VEL+p.IMU_vel_x*3.6
        #     print("-----------------------------",p.IMU_vel_x)
        
        # last_ENC_VEL = ENC_vel
    ...
    measurement_errors = ENC_plot - GPS_plot
