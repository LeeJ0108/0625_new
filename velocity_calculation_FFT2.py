...
class Position:
    ...
    def erp_Velocity(self):
        if self.erp_init_Count == 0:
            ...
            if self.last_enc !=  self.enc:
                ...
                self.sensor_data.append(self.erp_velocity2) # 실제 센서 데이터에 노이즈 추가
    ...
    def erp_filter(self):
        ...
        if len(self.sensor_data) > 3:
            fft_result = np.fft.fft(self.sensor_data)

            fs = sum(self.dt_array)/10
            freqs = np.fft.fftfreq(len(self.sensor_data), 1/fs)
            ...
            self.filtered_signal = np.fft.ifft(fft_result)
...
def main():
    ...
    while not rospy.is_shutdown():  
        ...
        filtered_ENC = p.erp_filter()
        if filtered_ENC != [ ]:
            print("---------------filtered-----------------",filtered_ENC)
            filter_velocity = filtered_ENC[-1]
            # F.extend(filtered_ENC)
           
            filtered_ENC = []
        
        F.append(filter_velocity)
        if count > 100000000:
            break
    ...
    plt.plot(X[start:end], F[start:end], label='Filtered Sensor Data', linestyle='--', linewidth=2,color='orange')
