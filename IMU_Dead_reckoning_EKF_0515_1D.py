    ...
    def IMU_velocity_calc(self,flag):
        if self.IMU_init_Count == 0:
            ...
            if IMU_time_intervel >= 0.001:  
                ...
                accel =np.sqrt(((self.linear_x2+self.IMU_last_accel_x)/2)**2 + ((self.linear_y2+self.IMU_last_accel_y)/2)**2 )
        return accel,self.IMU_vel_total
...
def dynamics_jacobian(state, dt): #미분 
    # Example: Jacobian of linear dynamics
    # return np.array([[1, dt],
    #             [0, 1]])

    return np.array([[1],[0]]) #2x1

# Define measurement function
def measurement_function(state):
    # Example: Linear measurement
    return np.array([state[0],state[1]]) #1x2
...
def main():
    ...
    process_noise_cov = np.array([[1e-4, 0],
                                [0, 1e-4]])  ###########프로세스 노이즈
    measurement_noise_cov = np.array([[0.1]])  ###########측정 노이즈

    # 확장 칼만 필터 인스턴스 생성
    ekf = ExtendedKalmanFilter_IMU_ENCODER(initial_state, initial_covariance, process_noise_cov, measurement_noise_cov)
